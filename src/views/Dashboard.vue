<template>
   <div class="wrap">
      <aside class="aside" :class="{ 'mobile-detail-view': selectedParkingDetail }">
         ...
         <div
            v-if="!store.state.parking.selectedParkingDetail && !store.state.parking.bookingView && $route.name !== 'Cabinet' &&  $route.name !== 'EditProfile'">



            <Filters v-if="!selectedParkingDetail" />

            <ParkingList v-if="!selectedParkingDetail &&  $route.name !== 'EditProfile'" :allParkings="allParkings"
               :selectedParkingDetail="selectedParkingDetail" @go-to-parking="goToParking"
               @show-parking-detail="showParkingDetail" />
         </div>

         <ParkingDetail v-else-if="store.state.parking.selectedParkingDetail &&  $route.name !== 'Cabinet'" :parking="selectedParking"
            @build-route="buildRoute" @book-parking="bookParking" @close-detail="closeParkingDetail" />
         <BookingView v-else-if="store.state.parking.bookingView &&  $route.name !== 'EditProfile'" :parking="selectedParking"
            @close="store.commit('parking/SET_BOOKING_VIEW', false)" />

         <router-view v-if="$route.name === 'Cabinet' || $route.name === 'EditProfile'" name="aside" />
      </aside>

      <router-link class="profile" :to="{ name: 'Cabinet' }">
         <img style="width: 40px; aspect-ratio: 1; margin-top: 10px; margin-right: 10px;"
